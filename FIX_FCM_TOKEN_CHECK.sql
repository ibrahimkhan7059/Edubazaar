-- Fix for FCM token check in trigger
-- Update the trigger function to properly check FCM tokens

CREATE OR REPLACE FUNCTION notify_chat_on_message() RETURNS TRIGGER AS $$
DECLARE
    conversation_data jsonb;
    sender_profile jsonb;
    recipient_id uuid;  -- Changed from text to uuid
    service_key text;
    ...
    fcm_token_count integer;  -- Added for debugging
BEGIN
    ...
    -- Check if recipient has FCM token (improved query)
