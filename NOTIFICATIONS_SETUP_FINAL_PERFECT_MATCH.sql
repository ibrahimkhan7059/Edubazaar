-- Complete Notification System Setup for Supabase - PERFECT FLUTTER MATCH
...
-- Ensure conversations table exists with correct structure - PERFECTLY MATCHING FLUTTER MODEL
CREATE TABLE IF NOT EXISTS conversations (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    participant_1_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,  -- matches participant_1_id
    participant_2_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,  -- matches participant_2_id
    listing_id UUID,  -- ADDED: Missing column for listing reference
    last_message_id UUID,  -- matches last_message_id
    last_message_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),  -- matches last_message_at
    participant_1_unread_count INTEGER DEFAULT 0,  -- matches participant_1_unread_count
    participant_2_unread_count INTEGER DEFAULT 0,  -- matches participant_2_unread_count
    is_active BOOLEAN DEFAULT true,  -- ADDED: Missing column for conversation status
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),  -- matches created_at
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),  -- matches updated_at
    UNIQUE(participant_1_id, participant_2_id)
);

-- Ensure messages table exists with correct structure - PERFECTLY MATCHING FLUTTER MODEL
CREATE TABLE IF NOT EXISTS messages (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,  -- matches id
    conversation_id UUID REFERENCES conversations(id) ON DELETE CASCADE,  -- matches conversation_id
    sender_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,  -- matches sender_id
    message_text TEXT NOT NULL,  -- matches message_text
    message_type TEXT DEFAULT 'text' CHECK (message_type IN ('text', 'image', 'listing_share', 'system')),  -- matches message_type
    attachment_url TEXT,  -- matches attachment_url
    listing_reference_id TEXT,  -- matches listing_reference_id
    is_read BOOLEAN DEFAULT false,  -- matches is_read
    is_edited BOOLEAN DEFAULT false,  -- matches is_edited
    edited_at TIMESTAMP WITH TIME ZONE,  -- matches edited_at
    is_delivered BOOLEAN DEFAULT false,  -- matches is_delivered
    delivered_at TIMESTAMP WITH TIME ZONE,  -- matches delivered_at
    read_at TIMESTAMP WITH TIME ZONE,  -- matches read_at
    reactions JSONB DEFAULT '{}',  -- matches reactions
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),  -- matches created_at
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()  -- matches updated_at
);
...
-- Create the main trigger function with PERFECT FLUTTER MODEL MATCH
...
/*
=== PERFECT FLUTTER MODEL MATCH VERIFICATION ===

CONVERSATIONS TABLE MATCHES:
✅ id
✅ participant_1_id
✅ participant_2_id  
✅ listing_id (ADDED)
