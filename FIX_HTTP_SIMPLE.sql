-- Simple HTTP fix - using basic http extension functions
-- Replace the trigger function with simplified HTTP approach
...
BEGIN
    ...
    -- Construct the payload as JSON string
    ...
    -- Make HTTP request using basic http extension
    BEGIN
        -- Use simple HTTP POST with curl-like syntax
        SELECT content INTO http_result
        FROM http((
            'POST',
            edge_function_url,
            ARRAY[
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        INSERT INTO logs (event_type, details) 
        VALUES ('http_request_failed', json_build_object(
            'error', SQLERRM,
            'message_id', NEW.id,
            'url', edge_function_url
        ));
