-- =====================================================
-- SAFE COMMUNITY SETUP - CHECK BEFORE DELETE
-- =====================================================

-- STEP 1: SAFELY DELETE EXISTING TABLES (IF THEY EXIST)
-- =====================================================

-- Drop triggers first (if they exist) - with IF EXISTS
DO $$ 
BEGIN
    -- Drop triggers if they exist
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_study_groups_updated_at') THEN
        DROP TRIGGER IF EXISTS update_study_groups_updated_at ON study_groups;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_group_posts_updated_at') THEN
        DROP TRIGGER IF EXISTS update_group_posts_updated_at ON group_posts;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_forum_topics_updated_at') THEN
        DROP TRIGGER IF EXISTS update_forum_topics_updated_at ON forum_topics;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_forum_replies_updated_at') THEN
        DROP TRIGGER IF EXISTS update_forum_replies_updated_at ON forum_replies;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_community_events_updated_at') THEN
        DROP TRIGGER IF EXISTS update_community_events_updated_at ON community_events;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_study_resources_updated_at') THEN
        DROP TRIGGER IF EXISTS update_study_resources_updated_at ON study_resources;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_forum_topic_reply_count') THEN
        DROP TRIGGER IF EXISTS update_forum_topic_reply_count ON forum_replies;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_event_participant_count_trigger') THEN
        DROP TRIGGER IF EXISTS update_event_participant_count_trigger ON event_participants;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_resource_like_count_trigger') THEN
        DROP TRIGGER IF EXISTS update_resource_like_count_trigger ON resource_likes;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_resource_download_count_trigger') THEN
        DROP TRIGGER IF EXISTS update_resource_download_count_trigger ON resource_downloads;
    END IF;
    
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_group_member_count_trigger') THEN
...
-- Drop tables in reverse order (child tables first) - with IF EXISTS
...
-- Resources indexes
CREATE INDEX idx_study_resources_uploader_id ON study_resources(uploader_id);
CREATE INDEX idx_study_resources_subject ON study_resources(subject);
CREATE INDEX idx_study_resources_created_at ON study_resources(created_at);
CREATE INDEX idx_study_resources_academic_level ON study_resources(academic_level);
...
-- 
-- All community tables have been created successfully!
