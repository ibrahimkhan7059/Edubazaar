-- ================================================
-- FIXED PUSH NOTIFICATION SETUP FOR EDUBAZAAR
-- ================================================
-- This script safely handles existing functions and policies

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Clean up existing objects safely - INCLUDING FUNCTIONS
...
-- Insert/Update service role key for Edge Function authentication
...
DO $$
BEGIN
    RAISE NOTICE '✅ FIXED push notification setup completed successfully!';
    RAISE NOTICE '📋 Tables created/verified: app_config, logs, user_fcm_tokens, user_notification_settings, notification_queue';
    RAISE NOTICE '🔧 Functions created FRESH: get_pending_notifications, mark_notification_sent, notify_chat_on_message, cleanup_old_logs';
    ...
    RAISE NOTICE '   - FCM tokens: %', (SELECT COUNT(*) FROM user_fcm_tokens);
    RAISE NOTICE '   - Notification settings: %', (SELECT COUNT(*) FROM user_notification_settings);
    RAISE NOTICE '   - Notification queue: %', (SELECT COUNT(*) FROM notification_queue);
    RAISE NOTICE '   - Chat notifications: %', (SELECT COUNT(*) FROM chat_notifications);
END $$; 